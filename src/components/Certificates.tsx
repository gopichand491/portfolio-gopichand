          ...
          <AnimatePresence mode="popLayout">
            {filteredCertificates.map((cert) => (
              <motion.button
                ...
                className="glass-card p-4 text-left group hover:border-accent/30 transition-all duration-300 cursor-pointer flex flex-col justify-between h-full"
              >
                <div>
                  {/* Thumbnail Wrapper */}
                  <div className="relative w-full h-36 mb-4.5 rounded-lg overflow-hidden bg-dark-700 border border-white/5 select-none">
                    <img
                      src={cert.image}
                      alt={`${cert.title} — ${cert.issuer}`}
                      className="w-full h-full object-cover object-top opacity-75 group-hover:opacity-100 group-hover:scale-[1.02] transition-all duration-300"
                      loading="lazy"
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-dark-900/90 via-transparent to-transparent" />
                    
                    {/* Hover eye icon badge */}
                    <div className="absolute bottom-2.5 right-2.5 p-1.5 bg-black/60 rounded-md border border-white/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      <Eye size={12} className="text-white" />
                    </div>

                    {/* Category Label badge */}
                    {cert.category && (
                      <span className="absolute top-2.5 left-2.5 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider bg-black/60 text-slate-300 border border-white/5 rounded">
                        {cert.category}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-start gap-2.5">
                    <Award size={15} className="text-accent-light mt-0.5 flex-shrink-0" />
                    <div>
                      <h4 className="text-sm font-bold text-white group-hover:text-accent-light transition-colors line-clamp-1 leading-snug">
                        {cert.title}
                      </h4>
                      <p className="text-xs text-slate-500 font-medium mt-0.5">{cert.issuer}</p>
                    </div>
                  </div>
                </div>
              </motion.button>
